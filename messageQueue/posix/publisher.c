 ...
 *  O_CREAT
 * */
    message_queue_id = mq_open(QUEUE_NAME, O_RDWR | O_EXCL | O_CREAT  | O_CREAT | O_EXCL, S_IRWXU | S_IRWXG, NULL);
    if (message_queue_id == -1) {
	 if(errno == EEXIST){
         message_queue_id = mq_open(QUEUE_NAME, O_RDWR | O_EXCL | O_EXCL, S_IRWXU |S_IRWXG, NULL);
	 }
